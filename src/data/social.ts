export const socialPrompts = [
    ...
    prompt: `Think like an ecommerce social media specialist and write a 90 character [channel] page description for a [industry] brand named [brand] which sells [product]`,
    author: 'ChatGPT',
    publish: '2025-06-17',
    category: 'Social'
  },
  ...
  {
    title: 'Instagram Reels Topic Brainstorming',
    prompt: `Brainstorm topics for an Instagram reel about [topic]. Execute this task as follows:

1. Understand the overarching theme of [topic] and its relevance to the target audience. This involves researching popular trends, current events, and audience preferences related to [topic].
2. Generate a list of potential subtopics or angles that can be explored within [topic]. Consider aspects that are visually appealing, thought-provoking, or trending on social media.
...
4. Provide a final selection of 5-10 topics with brief descriptions, highlighting what makes each idea compelling and how it can be visually presented in a short Instagram reel.`,
  ...
  {
    title: 'Instagram Reels Audience Engagement',
    prompt: `Suggest interactive questions to ask viewers of IG reels about [topic]. You will provide a set of engaging and thought-provoking questions that prompt viewers to interact with the content and share their opinions. This task involves the following steps:

1. Understand the key aspects and objectives of [topic]. This includes identifying the core themes, current trends, and potential points of interest that would resonate with the audience.
2. Brainstorm a list of open-ended questions that relate directly to [topic]. These questions should encourage viewers to think critically, share personal experiences, or express their preferences.
...
4. Provide a final selection of questions, ensuring that they vary in complexity and appeal to different viewer demographics. Additionally, include suggestions on how these questions can be integrated into an IG reel to maximize engagement and responses.`,
  ...
  {
    title: 'Instagram Reels Series Creator',
    prompt: `Create a 5-part series on [topic] for IG reel. Execute this task as follows:

1. Research [topic] to understand its key components, current trends, and audience interests. Identify subtopics that can be covered in a series to provide value and engage viewers.
2. Plan the series outline, determining the sequence of subtopics to be covered in each part. Ensure a logical flow that maintains audience interest and encourages them to watch subsequent parts.
3. Develop a storyboard for each part, outlining the main points to be discussed and any visual elements or text overlays that will enhance viewer understanding.
...
5. Publish the series on Instagram Reels, following a consistent schedule to maximize audience reach and engagement. Monitor the performance of each part and adjust future content based on viewer feedback and analytics.`,
  ...
  },
  {
    ...
    prompt: `Write a detailed script for an Instagram reel about [topic]. You'll perform this task as follows:
...
5. Provide suggestions for visual elements or actions that can accompany each part of the script, enhancing the overall impact and viewer retention.`,
  ...
  {
    title: 'Instagram Reels Humor Generator',
    prompt: `Add a funny pun to a video about [topic]. Execute this task as follows:

1. Understand video content, tone, and style. Note any recurring themes, phrases, or visual elements that can be used to create a relevant pun.
2. Brainstorm potential puns that align with the video's subject matter and are likely to resonate with the audience. Consider wordplay, double meanings, or references that add humor without being offensive or off-topic.
3. Evaluate each pun based on its originality, appropriateness, and potential to enhance the video's entertainment value. Select the pun that best fits these criteria and seamlessly integrates with the video's narrative.
...
5. Review the video with the added pun to confirm that it enhances the comedic appeal and does not disrupt the overall flow or message. Make any necessary adjustments for timing or delivery to optimize the pun's impact.`,
  ...
  {
    title: 'Instagram Reels Caption Writer',
    prompt: `Write a caption for an Instagram reel about a specific [topic]. The steps are as follows:

1. Understand the content and purpose of the Instagram reel. This includes knowing the topic, the tone of the reel, and the key message that needs to be conveyed.
2. Identify the target audience for the Instagram reel. Consider their demographics, interests, and the type of content that typically resonates with them on the platform.
3. Brainstorm a list of potential caption ideas that align with the content of the reel and are likely to engage the target audience. This may involve wordplay, questions, or calls to action.
...
5. Write the chosen caption, ensuring it is concise, attention-grabbing, and complements the visual content of the reel.`,
  ...
  {
    title: 'Instagram Reels Trend Analyzer',
    prompt: `Analyze and suggest trending topics for IG reels in [niche]. You'll perform this task as follows:

1. Understand the [niche] industry: Before suggesting topics, it's crucial to have a solid grasp of the [niche]. This involves its current trends, popular content creators, and the audience demographics that engage with this content.
2. Research current trending topics: Utilize Instagram, TikTok, YouTube, and other relevant platforms to identify the content that is currently performing well in the [niche]. Look for common themes, viral challenges, or emerging discussions.
3. Analyze engagement metrics: For the identified content, delve into the engagement metrics. Pay attention to the number of views, likes, comments, and shares. This data can indicate not only what is currently trending but also what is resonating with the audience.
...
5. Suggest a list of trending topics: Based on your research and analysis, compile a list of trending topics for IG reels in [niche]. For each topic, provide a brief explanation of why it is relevant and how creators can approach it to maximize engagement.`,
  ...
  },
  {
    ...
    prompt: `Generate topics for a podcast episode about [topic]. You'll perform this task as follows:

1. Understand the overarching theme and purpose of the podcast. This includes knowing the target audience, the tone of the podcast, and its educational or entertainment value.
2. Research [topic] extensively to identify various subtopics, current trends, controversies, and related news. Utilize reputable sources such as academic journals, industry publications, and expert interviews.
...
4. Prioritize the generated topics based on their relevance, novelty, and potential audience interest. Provide a diverse range of ideas that can cater to different listener preferences or knowledge levels.`,
  ...
  {
    title: 'Create Podcast Episode Title',
    prompt: `Generate catchy titles for a podcast episode about [topic]. You'll complete this task as follows:

1. Understand the key themes, discussions, and insights of the podcast episode about [topic]. This involves analyzing the content, main points, and the overall tone of the discussion.
2. Brainstorm a list of keywords and phrases that encapsulate the episode's content and its most engaging aspects. Consider what would grab the audience's attention and accurately represent the episode.
...
4. Prioritize the titles based on their ability to attract listeners, their relevance to the content, and their overall appeal.`,
  ...
  {
    title: 'Suggest Podcast Interactive Content Ideas',
    prompt: `Suggest interactive content ideas to increase listener engagement during a podcast centered around [topic]. You'll perform this task as follows:

1. Understand the core theme and objectives of the podcast centered around [topic]. This involves listening to previous episodes, reviewing audience feedback, and identifying key areas of interest within the [topic].
2. Brainstorm interactive content ideas that align with the podcast's tone and goals. Consider various formats such as:
   - Live Q&A sessions with experts or guests featured on the podcast.
   - Polls or surveys related to upcoming episode topics or audience preferences.
   - Interactive storytelling where listeners contribute to the narrative or make decisions that influence the direction of a story.
   - Virtual workshops or tutorials that complement podcast content, providing hands-on learning experiences.
...
4. Prioritize the interactive content ideas, providing a shortlist with a brief explanation of each idea and how it enhances listener engagement while complementing the podcast's main content.`,
  ...
  {
    title: 'Create Podcast Social Media Content',
    prompt: `Generate a week's worth of engaging tweets to promote a podcast about [topic]. Execute this task as follows:

1. Understand the podcast's content, tone, and target audience. This involves researching the [topic], listening to previous podcast episodes, and reviewing any existing promotional material.
2. Brainstorm a variety of tweet angles that highlight different aspects of the podcast. These could include intriguing questions, quotes from episodes, behind-the-scenes insights, or interactive polls.
...
4. Provide a schedule for when each tweet should be posted, taking into account optimal times for audience engagement and the overall narrative you want to convey throughout the week.`,
  ...
  {
    title: 'Write Podcast Email Newsletter Content',
    prompt: `Write an email newsletter to inform subscribers about a podcast episode featuring [guest speaker]. Execute this task as follows:

1. Identify the key information to include in the newsletter: the guest speaker's name, their expertise or significance, the podcast episode's title, a brief overview of what will be discussed, and the airing date.
2. Craft a subject line that is engaging and clearly indicates the content of the email. The subject line should entice recipients to open the email.
3. Begin the newsletter with a friendly greeting to create a personal connection with the subscribers.
4. Introduce the guest speaker, highlighting their credentials and why they are a valuable addition to the podcast.
5. Provide a teaser of the episode content without giving away too much, emphasizing any exclusive insights or exciting discussions.
6. Include details on when the episode will be available for listening and on which platforms.
...
8. Conclude the email with a call to action, such as subscribing to the podcast if they haven't already or sharing the episode with friends who might be interested.`,
  ...
  {
    title: 'Draft a Podcast Ad Script',
    prompt: `Write a 30-second ad script for a [product] that will be promoted on a podcast episode. Execute this task as follows:

1. Identify the key features and benefits of the [product]. Understand what sets it apart from similar products in the market and why it would appeal to the podcast's audience.
2. Craft a concise and engaging script that highlights these key features while also aligning with the tone and style of the podcast. The script should be attention-grabbing, informative, and persuasive.
...
4. Review the script to ensure it fits within the 30-second time frame while delivering a compelling message. Make any necessary adjustments to optimize impact within the time constraint.`,
  ...
  },
  {
    ...
    prompt: `Write a podcast episode script about [topic]. You'll perform this task as follows:

1. Choose a compelling and relevant [topic] for the podcast episode. Consider current trends, audience interests, and the overall theme of the podcast.
2. Research the chosen [topic] extensively. Look for recent studies, news articles, expert opinions, and any other relevant information that can provide depth and insight into the subject.
3. Outline the script, including an introduction that hooks the audience, main discussion points that cover different aspects of the [topic], and a conclusion that summarizes key takeaways or sets up further discussion.
...
5. Review and edit the script for clarity, flow, and accuracy of information. Make sure that the script is tailored to the podcast's style and target audience, providing value and interest throughout.`,
  ...
  {
    title: 'Create Podcast Interview Questions',
    prompt: `Generate a list of questions for a podcast interview with an expert on [topic]. You'll complete this task as follows:
...
5. Provide a finalized list of questions, organized by section, ready for use in the podcast interview.`,
  ...
  {
    title: 'Help With Podcast Guest Outreach',
    prompt: `Write an email inviting a [guest] to join a podcast episode. Execute this task as follows:

1. Understand the purpose and theme of the podcast episode. This will help in tailoring the invitation to the [guest] and explaining why their expertise or experience is relevant.
2. Identify the contact information of the [guest], including their email address and any preferred communication methods if available.
3. Begin the email with a polite greeting and a brief introduction to yourself and the podcast. Mention why you are reaching out to them specifically, citing any relevant work or achievements that make them a desirable guest.
4. Provide details about the podcast, such as its audience size, typical topics covered, and the format of the episode they are being invited to (interview, panel discussion, etc.).
...
6. Close the email with a friendly sign-off, reiterating the appreciation for their work and the interest in having them on the podcast.`,
  ...
  {
    title: 'Help With Podcast Sponsor Outreach',
    prompt: `Write an email offering a sponsorship opportunity for a podcast. You'll compose this email as follows:

1. Write a subject line that is attention-grabbing and clearly states the purpose of the email.
2. Start with a polite greeting, addressing the recipient by name if possible.
3. Introduce yourself or the company you represent. Briefly explain your role and your enthusiasm for their podcast. Mention a specific episode or segment that particularly resonated with you to personalize the introduction.
4. Transition from the introduction to the sponsorship offer. Explain that you see great potential for a partnership and outline what the sponsorship would entail. This includes details such as the type of promotion (pre-roll, mid-roll, or post-roll mentions), potential reach of the podcast, and any demographic information that might be relevant to the recipient's brand.
5. Detail the benefits to the recipient. This could include increased visibility to a specific target audience, potential for increased engagement or sales, and any successful past partnerships you've had with similar podcasts.
...
7. End the email with a polite closing. Thank them for considering the opportunity and sign off with your name and position.`,
  ...
  {
    title: 'Generate TikTok Video Ideas',
    prompt: `I need help coming up with creative and engaging TikTok video ideas centered around a specific topic. The videos should be unique, attention-grabbing, and tailored to resonate with the TikTok audience.

Your task is to brainstorm and outline five distinct TikTok video concepts that are both entertaining and informative. Each idea should be innovative and designed to captivate viewers on the platform.
...
- Topic: [topic]`,
  ...
  {
    title: 'Write TikTok Script',
    prompt: `I need a script for a TikTok video that will engage viewers and effectively convey information on a specific topic. The script should be concise, entertaining, and optimized for the short video format of TikTok.

Your task is to write a script that captures the audience's attention from the start, delivers the key points effectively, and encourages viewers to engage with the content (likes, comments, shares).
...
- Key points to cover: [key points]`,
  ...
  {
    title: 'Analyze Current TikTok Trends',
    prompt: `I need someone to analyze the current trending hashtags on TikTok related to a specific topic. The analysis should identify the most popular and relevant hashtags that are currently driving engagement and visibility on the platform.

Your task is to research and compile a list of trending hashtags on TikTok that are related to the specified topic. Provide insights into why these hashtags are popular and how they can be effectively utilized to increase reach and engagement.
...
- Topic: [topic]`,
  ...
  {
    title: 'Write Creative TikTok Captions',
    prompt: `I need a catchy and engaging caption for a TikTok video that will capture the attention of viewers and encourage them to watch the video until the end. The caption should be creative, intriguing, and relevant to the topic of the video.

Your task is to come up with a captivating caption that will entice viewers to click on the video and engage with the content.
...
- Video topic: [topic]`,
  ...
  {
    title: 'Get Feedback on TikTok Content',
    prompt: `I need someone to review my TikTok script and provide feedback on how it can be improved to increase engagement and appeal to the target audience. The script is for a short video intended to be entertaining and shareable on the platform.

Your task is to carefully read through the script and offer suggestions for enhancing the content, making it more engaging, and ensuring it aligns with the TikTok format and audience preferences.
...
- TikTok script: [script]`,
  ...
  {
    title: 'Automate TikTok Comment Responses',
...
- Target audience: [target audience]`,
  ...
  },
  {
    ...
    prompt: `I need help coming up with a fun and engaging TikTok challenge that I can start on my channel. The challenge should be related to a specific topic or theme that resonates with my audience and encourages participation and sharing.

Your task is to brainstorm ideas for a TikTok challenge that is creative, interactive, and has the potential to go viral. Provide a brief description of the challenge, including any specific rules or instructions for participants.
